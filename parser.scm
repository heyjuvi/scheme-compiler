...
(define (myquoted? token) (eq? token 'myquote-token))
(define (quasimyquoted? token) (eq? token 'quasimyquote-token))
(define (unmyquoted? token) (eq? token 'unmyquote-token))
...
(define (parse-expr)
  (let ((next-token (read-token)))
    (cond ((left-parenthesis? next-token)
           (parse-list '()))
          ((myquoted? next-token)
           (list 'myquote (parse-expr)))
          ((quasimyquoted? next-token)
           (list 'quasimyquote (parse-expr)))
          ((unmyquoted? next-token)
           (list 'unmyquote (parse-expr)))
...
(define (parse-list collector)
  (let ((next-token (read-token)))
    (cond ((left-parenthesis? next-token)
          ...
          ((myquoted? next-token)
           (parse-list
             (cons (list 'myquote (parse-expr))
                   collector)))
          ((quasimyquoted? next-token)
           (parse-list
             (cons (list 'quasimyquote (parse-expr))
                   collector)))
          ((unmyquoted? next-token)
           (parse-list
             (cons (list 'unmyquote (parse-expr))
                   collector)))
          ((boolean-token? next-token)
           (parse-list
