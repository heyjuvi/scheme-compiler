  ...
  (let ((op (primcall-operator x))
    ...
    (cond
      ...
      ((eq? op 'bit-or)
       ...
       (emit-call2 "prim_fixnum_bit_or" tmp1 tmp2 var))
      ((eq? op 'bit-and)
       (emit-expr (primcall-operand1 x) tmp1 env)
       (emit-expr (primcall-operand2 x) tmp2 env)
       (emit-call2 "prim_fixnum_bit_and" tmp1 tmp2 var))
      ((eq? op 'shift-left)
       (emit-expr (primcall-operand1 x) tmp1 env)
       (emit-expr (primcall-operand2 x) tmp2 env)
       (emit-call2 "prim_fixnum_shift_left" tmp1 tmp2 var))
      ((eq? op 'shift-right)
       (emit-expr (primcall-operand1 x) tmp1 env)
       (emit-expr (primcall-operand2 x) tmp2 env)
       (emit-call2 "prim_fixnum_shift_right" tmp1 tmp2 var))
      ((eq? op '+)
       (emit-expr (primcall-operand1 x) tmp1 env)
       (emit-expr (primcall-operand2 x) tmp2 env)
      ...
      ((eq? op '<)
       (emit-expr (primcall-operand1 x) tmp1 env)
       (emit-expr (primcall-operand2 x) tmp2 env)
       (emit-call2 "prim_fixnum_less" tmp1 tmp2 var))
      ((eq? op '<=)
       (emit-expr (primcall-operand1 x) tmp1 env)
       (emit-expr (primcall-operand2 x) tmp2 env)
       (emit-call2 "prim_fixnum_less_equal" tmp1 tmp2 var))
      ((eq? op '>)
       (emit-expr (primcall-operand1 x) tmp1 env)
       (emit-expr (primcall-operand2 x) tmp2 env)
       (emit-call2 "prim_fixnum_greater" tmp1 tmp2 var))
      ((eq? op '>=)
       (emit-expr (primcall-operand1 x) tmp1 env)
       (emit-expr (primcall-operand2 x) tmp2 env)
       (emit-call2 "prim_fixnum_greater_equal" tmp1 tmp2 var))
      ((eq? op 'bin-and)
       (emit-expr (primcall-operand1 x) tmp1 env)
       (emit-expr (primcall-operand2 x) tmp2 env)
      ...
      ((eq? op 'eq?)
       (emit-expr (primcall-operand1 x) tmp1 env)
       (emit-expr (primcall-operand2 x) tmp2 env)
       (emit-call2 "prim_generic_eq" tmp1 tmp2 var))
      ((eq? op 'equal?)
       (emit-expr (primcall-operand1 x) tmp1 env)
       (emit-expr (primcall-operand2 x) tmp2 env)
      ...
      ((eq? op 'string-ref)
       (emit-expr (primcall-operand1 x) tmp1 env)
       (emit-expr (primcall-operand2 x) tmp2 env)
       (emit-call2 "prim_string_ref" tmp1 tmp2 var))
      ((eq? op 'string-append)
       (emit-expr (primcall-operand1 x) tmp1 env)
       (emit-expr (primcall-operand2 x) tmp2 env)
      ...
      ((eq? op 'symbol->string)
       (emit-expr (primcall-operand1 x) tmp1 env)
       (emit-call1 "prim_symbol_to_string" tmp1 var))
      ((eq? op 'char->string)
       (emit-expr (primcall-operand1 x) tmp1 env)
       (emit-call1 "prim_char_to_string" tmp1 var))
      ((eq? op 'any->string)
       (emit-expr (primcall-operand1 x) tmp1 env)
      ...
      ((eq? op 'open-input-file)
       (emit-expr (primcall-operand1 x) tmp1 env)
       (emit-call1 "prim_port_open_input_file" tmp1 var))
      ((eq? op 'open-output-file)
       (emit-expr (primcall-operand1 x) tmp1 env)
       (emit-call1 "prim_port_open_output_file" tmp1 var))
      ((eq? op 'open-i/o-file)
       (emit-expr (primcall-operand1 x) tmp1 env)
       (emit-call1 "prim_port_open_i_o_file" tmp1 var))
      ((eq? op 'eof-object?)
       (emit-expr (primcall-operand1 x) tmp1 env)
       (emit-call1 "prim_port_is_eof_object" tmp1 var))
      ((eq? op 'read-char)
       (emit-expr (primcall-operand1 x) tmp1 env)
       (emit-call1 "prim_port_read_char" tmp1 var))
      ((eq? op 'argc)
      ...
      ((eq? op 'exit)
       (emit-expr (primcall-operand1 x) tmp1 env)
       (emit-call1 "prim_process_exit" tmp1 var))
...
(define (emit-c-env_ c-env-var free-vars n env)
  (let ((tmp1 (unique-var))
	...
	(emit-immediate n tmp1)
        (emit (format "  ~A = call i64 @prim_list_ref(i64 ~A, i64 ~A)" (list tmp2 c-env-var tmp1)))
...
(define (emit-function x env)
  (let* ((name (function-name->ll-name (function-name x)))
    ...
    (emit (format "define i64 @~A(~A) {" (list name (args-string vars))))
...
(define (emit-global-var global-var)
  (emit (format "~A = global i64 0, align 8" (list global-var))))
...
(define (emit-closure x var env)
  ...
  (let* ((name (function-name->ll-name (closure-function x)))
    ...
    (emit (format "  ~A = ptrtoint i64(~A)* @~A to i64"
		  (list tmp1
		        signature
		        name)))
...
(define (emit-string_ bytes var)
  (let ((tmp1 (unique-var))
	...
	(tmp4 (unique-var)))
    (emit-alloca tmp1)
    (emit (format "  ~A = bitcast i64* ~A to i8*" (list tmp2 tmp1)))
    (for-each
      (lambda (i)
        (let ((tmp-for (unique-var))
  	      (byte (char->fixnum (list-ref bytes i))))
          (emit (format "  ~A = getelementptr i8, i8* ~A, i64 ~A" (list tmp-for tmp2 i)))
          (emit (format "  store i8 ~A, i8* ~A" (list byte tmp-for)))))
      (iota (min (length bytes) 8)))
    (if (< (length bytes) 8)
      (begin
        (emit (format "  ~A = getelementptr i8, i8* ~A, i64 ~A" (list tmp4 tmp2 (length bytes))))
        (emit (format "  store i8 ~A, i8* ~A" (list 0 tmp4))))
      'last-block)
    (emit-load tmp3 tmp1)
    (emit (format "  ~A = call i64 @___reserved_heap_store_i64(i64 ~A)" (list var tmp3)))
    (if (>= (length bytes) 8)
...
(define (emit-string x var env)
  (let ((bytes (string->list x))
    ...
    (emit (format "  ~A = load i64, i64* @prim_string_tag" (list tmp2)))
    (emit (format "  ~A = or i64 ~A, ~A" (list var tmp1 tmp2)))))
...
(define (emit-application x var env)
  (let* ((evaluated-list
    ...
    (emit (format "  ~A = inttoptr i64 ~A to i64(~A)*" (list func-ptr-var func-addr-var func-args-signature)))
    (emit (format "  ~A = call i64 ~A(~A)" (list var func-ptr-var func-args)))))
...
(define (codegen-main)
  (let* ((in-file (list-ref (argv) 1))
	 (ast (parse in-file)))
    ;(print-ast ast "")
    (let ((p-ast (map preprocess ast)))
      (extract-global-vars p-ast)
      (debug "GLOBAL ENV: ") (debug-newline)
      (debug global-env) (debug-newline) (debug-newline)
